...
module.exports = function(paths, encodePolylines) {
  ...
  return paths.map(function(path) {
    ...
    if (!Array.isArray(path.points) && !Array.isArray(path.enc)) {
      throw new Error('Each path must have an array of points or an array of enc (encoded polylines)');
    } else if (path.enc) {
      p.push( 'enc:' + path['enc'].join('|'));
    } else {
